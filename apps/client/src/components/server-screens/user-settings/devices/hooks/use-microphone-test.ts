...
const useMicrophoneTest = ({
    ...
    const setAudioLevelFromDecibels = useCallback(
    ...
    const startTestPipeline = useCallback(
        async (requestId: number) => {
            ...
            try {
                ...
                try {
                    meterWorkletNode = await createAudioMeterWorkletNode(
                        audioContext,
                        {
                            enabled: true,
                ...
                if (meterWorkletNode) {
                    ...
                    analyser.smoothingTimeConstant =
                        ANALYZER_SMOOTHING_TIME_CONSTANT;
